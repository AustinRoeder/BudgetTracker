...
using ARBudgetTracker.Providers;
using ARBudgetTracker.Results;
...
namespace ARBudgetTracker.Controllers
{
    ...
    public class AccountController : ApiController
    {
        ...
        public async Task<IHttpActionResult> Register(RegisterBindingModel model)
        {
            ...
            if (invited)
                return await JoinHousehold(model.Email, model.Code);
            return Ok("User has now been registered.");
        }
        ...
        {
            var user = db.Users.Where(u=>u.Email == inviteEmail).FirstOrDefault();
            var invite = db.Invitations.Where(i => i.Code == inviteCode && i.InvitedEmail == inviteEmail)
                                                .FirstOrDefault();
            if (invite != null)
            {
            ...
            return Ok("No invitation was found.");
        }
        ...
        public async Task<IHttpActionResult> CreateHousehold(string name)
        {
            ...
            {
                return Ok("Must leave current household in order to build a new one.");
            }
        ...
        private string GetRandomString(int length)
        {
            ...
            var rand = new Random();
            return new string(Enumerable.Range(0, length).Select(c => chars[rand.Next(chars.Length)])
              .ToArray());
        }
